...
IExchangeRateProvider rateProvider = new ExchangeRateJsonProvider();

var Rates = await rateProvider.GetExchangeRatesAsync();

foreach (var rate in Rates)
{
    Console.WriteLine($"{rate.CurrentCurrency} - {rate.TargetCurrency} = {rate.ExchangeRateValue}");
}